  ...
  v-row
    ...
    v-col(cols="12")
      v-form(v-model="valid" @submit.prevent="submitCart")
        v-text-field(v-model.number="quantity" type="number" lable="數量" :rules="[rules.required, rules.number]")
        v-btn(type="submit" color="primary") 加入購物車
  v-overlay.align-center.justify-center(:model-value="!product.sell" persistent)
    h1.text-center.text-red 已下架
